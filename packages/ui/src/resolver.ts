...
// 3. TmTable 走子入口 @trustmo/tm-ui/table：vxe-table 体积大，独立 chunk 隔离，按需加载不污染主入口。
// 4. 其余 TmXxx 走主入口 @trustmo/tm-ui：fail-fast 设计——未实现组件（如 TmXYZ）也返回 @trustmo/tm-ui，
//    业务侧 vite build 会因 `import { TmXYZ } from '@trustmo/tm-ui'` 的 named export 缺失立即报错，
...
/**
 * @trustmo/tm-ui 按需导入 Resolver 工厂
 *
 * @returns 结构性兼容 unplugin-vue-components ComponentResolverObject 的对象，
 *          业务方配 `resolvers: [TmResolver()]` 即可
 *
 * 映射约定：
 *   - TmButton / TmInput / TmSelect / TmForm / TmFormItem / TmConfigProvider / ... → from '@trustmo/tm-ui'
 *   - TmTable → from '@trustmo/tm-ui/table'（vxe 子入口，体积隔离）
...
export function TmResolver(): TmComponentResolverObject {
  return {
    ...
    resolve: (name: string) => {
      ...
      // TmTable 走子入口 @trustmo/tm-ui/table（vxe-table 体积大需独立 chunk），其余走主入口 @trustmo/tm-ui
      // 注：未实现组件（TmXYZ）也命中 '@trustmo/tm-ui' 分支——fail-fast，业务侧构建期即报错（brief Bug 5 决策）
      const from = kebab === 'table' ? '@trustmo/tm-ui/table' : '@trustmo/tm-ui'
