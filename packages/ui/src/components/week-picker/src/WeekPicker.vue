<!-- packages/ui/src/components/week-picker/src/WeekPicker.vue -->
<!--
  TmWeekPicker 薄封装组件：ant WeekPicker，与 TmDatePicker 同契约
  （运行时 props 声明原因、useReadonlyLock、valueFormat 桥接、disabled 级联均见
  date-picker/src/DatePicker.vue 文件头注；本组件固定周语义，无 picker prop）
-->
<script setup lang="ts">
import { computed, useSlots, type PropType } from 'vue'
import { WeekPicker as AWeekPicker } from 'ant-design-vue'
...
/** ant WeekPicker 实例类型（ant 未导出实例类型，用 InstanceType 推导） */
type WeekPickerInstance = InstanceType<typeof AWeekPicker>

defineOptions({ name: 'TmWeekPicker', inheritAttrs: false })
...
/** 方法透传：父组件 ref 可直接调用内部 ant WeekPicker 实例方法 */
const { innerRef, exposed } = useForwardRef<WeekPickerInstance>()
...
<template>
  <AWeekPicker ref="innerRef" v-bind="forwardBindings" v-model:value="inner">
