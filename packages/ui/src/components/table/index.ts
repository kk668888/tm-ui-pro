 ...
 * - 业务侧 `app.use(TmTable)` 或 `app.use(@trustmo/tm-ui)` 全局注册
 * - 也可直接当组件用：`import { TmTable } from '@trustmo/tm-ui'`
 */
export const TmTable = withInstall(Table, 'TmTable')

// 类型 re-export：业务侧 import type { TmTableProps, TmTableExtProps, VxeGridProps, ... } from '@trustmo/tm-ui'
