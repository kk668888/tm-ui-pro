...
export const tmPaginationDefaults = {
  // 显式标注返回类型：ant PaginationProps = Partial<ExtractPropTypes>（索引含 undefined），
  // withDefaults 的 InferDefault 要求无 undefined 的精确类型，否则 TS2345。
  showSizeChanger: (): boolean => DEFAULT_SHOW_SIZE_CHANGER,
  pageSizeOptions: (): string[] => [...DEFAULT_PAGE_SIZE_OPTIONS],
} as const
