...
// VitePress 自定义主题入口：扩展默认主题 + 真正注册 ant / vxe / @trustmo/tm-ui
//
// 设计变更（Task 15 / Bug 3 收口）：
// Task 14 时本文件作为 @trustmo/tm-ui dts portability 真实消费验证入口，仅把 TmButton /
...
//   4. @trustmo/tm-ui：app.use(install)（聚合 install，与业务方 app.use(@trustmo/tm-ui) 一致，
...
// 应用层依赖：ant + vxe + @trustmo/tm-ui（与 packages/ui 业务方用法一致）
import Antd from 'ant-design-vue'
import VxeUIPCUI from 'vxe-pc-ui'
import VxeUITable from 'vxe-table'
// @trustmo/tm-ui 的聚合 install：app.use(install) 一次性注册全部 Tm 组件
// （注意：这里 import 自 @trustmo/tm-ui 主入口，走 packages/ui/package.json exports 的 dist，
//  与业务方一致；不直接 import 源码，避免与 packages/ui 的内部 import 混淆。）
import { install as tmInstall } from '@trustmo/tm-ui'
...
const theme: Theme = {
  ...
  enhanceApp({ app }) {
    ...
    app.use(VxeUITable)
    // 4) @trustmo/tm-ui：聚合 install，全量注册 Tm 组件（与业务方 app.use(@trustmo/tm-ui) 一致）
    app.use(tmInstall)
